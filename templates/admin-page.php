...
<div class="wrap wp-watchdog-admin">
    <div class="wp-watchdog-section-header">
        <div>
            <h1 style="display:flex; align-items:center; gap:10px; margin:0;">
                ...
                <?php esc_html_e('Site Add-on Watchdog', 'Site-Add-on-Watchdog-main'); ?>
            </h1>
            <p class="wp-watchdog-muted"><?php esc_html_e('Monitor plugin health, history, and alerts from a single place.', 'Site-Add-on-Watchdog-main'); ?></p>
        </div>
        <div style="display:flex; gap:8px; align-items:center; flex-wrap:wrap;">
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <button class="button button-primary button-hero" type="submit">
                    ...
                    <?php esc_html_e('Run manual scan', 'Site-Add-on-Watchdog-main'); ?>
                </button>
            </form>
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                ...
                <button class="button button-secondary button-hero" type="submit">
                    ...
                    <?php esc_html_e('Send notifications now', 'Site-Add-on-Watchdog-main'); ?>
                </button>
            </form>
        </div>
    </div>
    ...
    <?php if (empty($watchdogCronSecretPersisted)) : ?>
        <div class="notice notice-warning"><p><?php esc_html_e('cron secret 尚未初始化或無法儲存，請重新保存設定', 'Site-Add-on-Watchdog-main'); ?></p></div>
    <?php endif; ?>
    ...
    <?php if (! empty($watchdogWpScanError) && is_array($watchdogWpScanError)) : ?>
        ...
        if ($watchdogWpScanMessage !== '' && $watchdogWpScanCode !== '') {
            $watchdogWpScanMessage = sprintf(
                ...
                __('WPScan error (%1$s): %2$s', 'Site-Add-on-Watchdog-main'),
                $watchdogWpScanCode,
    ...
    <?php if (! empty($watchdogNoticeFlags['updated'])) : ?>
        <div class="notice notice-success is-dismissible"><p><?php esc_html_e('Settings saved.', 'Site-Add-on-Watchdog-main'); ?></p></div>
    <?php endif; ?>

    <?php if (! empty($watchdogNoticeFlags['scan'])) : ?>
        <div class="notice notice-info is-dismissible"><p><?php esc_html_e('Manual scan completed.', 'Site-Add-on-Watchdog-main'); ?></p></div>
    <?php endif; ?>

    <?php if ($watchdogNotificationResult !== '') : ?>
        <?php if ($watchdogNotificationResult === 'sent') : ?>
            <div class="notice notice-success is-dismissible"><p><?php esc_html_e('Notifications were dispatched.', 'Site-Add-on-Watchdog-main'); ?></p></div>
        <?php elseif ($watchdogNotificationResult === 'throttled') : ?>
            <div class="notice notice-warning is-dismissible"><p><?php esc_html_e('Notifications skipped to avoid rapid re-sends. Please wait a moment and try again.', 'Site-Add-on-Watchdog-main'); ?></p></div>
        <?php elseif ($watchdogNotificationResult === 'unchanged') : ?>
            <div class="notice notice-info is-dismissible"><p><?php esc_html_e('No notification changes detected since the last send.', 'Site-Add-on-Watchdog-main'); ?></p></div>
        <?php endif; ?>
    <?php endif; ?>

    <?php if ($watchdogFailedNotificationStatus !== '') : ?>
        <?php if ($watchdogFailedNotificationStatus === 'resent') : ?>
            <div class="notice notice-success is-dismissible"><p><?php esc_html_e('Queued the captured notification payload for resend.', 'Site-Add-on-Watchdog-main'); ?></p></div>
        <?php elseif ($watchdogFailedNotificationStatus === 'missing') : ?>
            <div class="notice notice-error is-dismissible"><p><?php esc_html_e('No failed notification payload was available to resend.', 'Site-Add-on-Watchdog-main'); ?></p></div>
        <?php endif; ?>
    <?php endif; ?>
    ...
    <div class="wp-watchdog-grid">
        <div class="wp-watchdog-surface">
            <div class="wp-watchdog-section-title">
                ...
                <?php esc_html_e('Delivery health', 'Site-Add-on-Watchdog-main'); ?>
            </div>
                <?php $watchdogIsCronDisabled = ! empty($watchdogCronStatus['cron_disabled']); ?>
            <?php if ($watchdogIsCronDisabled) : ?>
                <p class="wp-watchdog-muted"><?php esc_html_e('WP-Cron appears disabled. Use a real cron job or the server endpoint below to keep scans running.', 'Site-Add-on-Watchdog-main'); ?></p>
                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" style="margin:12px 0;">
                    ...
                    <button class="button button-primary" type="submit">
                        ...
                        <?php esc_html_e('Retry notifications', 'Site-Add-on-Watchdog-main'); ?>
                    </button>
                </form>
            <?php else : ?>
                <p class="wp-watchdog-muted"><?php esc_html_e('WP-Cron is running. Watchdog will use scheduled scans and backup triggers to deliver alerts.', 'Site-Add-on-Watchdog-main'); ?></p>
            <?php endif; ?>
            <div class="wp-watchdog-divider"></div>
            <p class="wp-watchdog-muted" style="word-break:break-word;">
                <strong><?php esc_html_e('Server cron endpoint:', 'Site-Add-on-Watchdog-main'); ?></strong><br />
                <code><?php echo esc_html($watchdogCronEndpoint); ?></code>
            </p>
            <p class="wp-watchdog-muted"><?php esc_html_e('Call this URL from a system cron or monitoring service to trigger scans or notification retries even when wp-cron is disabled.', 'Site-Add-on-Watchdog-main'); ?></p>
            <div class="wp-watchdog-divider"></div>
            <div>
                <p class="wp-watchdog-muted" style="margin-bottom:8px;"><strong><?php esc_html_e('Queue status', 'Site-Add-on-Watchdog-main'); ?></strong></p>
                <?php
                ...
                <p class="wp-watchdog-muted">
                    ...
                    printf(
                        ...
                        esc_html__('Queued jobs: %d', 'Site-Add-on-Watchdog-main'),
                ...
                <?php if ($watchdogQueueLength > 0 && $watchdogQueueNextAttempt) : ?>
                    <p class="wp-watchdog-muted">
                        ...
                        printf(
                            ...
                            esc_html__('Next attempt: %1$s', 'Site-Add-on-Watchdog-main'),
                            esc_html(wp_date(get_option('date_format') . ' ' . get_option('time_format'), (int) $watchdogQueueNextAttempt))
                        );
                        ?>
                    </p>
                <?php else : ?>
                    <p class="wp-watchdog-muted"><?php esc_html_e('No pending notifications in the queue.', 'Site-Add-on-Watchdog-main'); ?></p>
                <?php endif; ?>
            </div>
            <div class="wp-watchdog-divider"></div>
            <div>
                <p class="wp-watchdog-muted" style="margin-bottom:8px;"><strong><?php esc_html_e('Captured notification payloads', 'Site-Add-on-Watchdog-main'); ?></strong></p>
                <?php if (! empty($watchdogLastFailedNotification)) : ?>
                    ...
                    $watchdogFailedChannel = $watchdogLastFailedNotification['description']
                        ?: ($watchdogLastFailedNotification['channel'] ?? __('Unknown channel', 'Site-Add-on-Watchdog-main'));
                    ...
                    <p class="wp-watchdog-muted">
                        ...
                        printf(
                            ...
                            esc_html__('Last failure recorded %1$s via %2$s.', 'Site-Add-on-Watchdog-main'),
                    ...
                    <div style="display:flex; gap:8px; flex-wrap:wrap;">
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                            ...
                            <button class="button button-primary" type="submit">
                                <span class="dashicons dashicons-controls-repeat" aria-hidden="true"></span>
                                <?php esc_html_e('Re-queue payload', 'Site-Add-on-Watchdog-main'); ?>
                            </button>
                        </form>
                        <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                            ...
                            <button class="button" type="submit">
                                ...
                                <?php esc_html_e('Download payload', 'Site-Add-on-Watchdog-main'); ?>
                            </button>
                        </form>
                    </div>
                <?php else : ?>
                    <p class="wp-watchdog-muted"><?php esc_html_e('No failed notification payload has been captured yet.', 'Site-Add-on-Watchdog-main'); ?></p>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <div class="wp-watchdog-grid">
        <div class="wp-watchdog-surface">
            <div class="wp-watchdog-section-title">
                ...
                <?php esc_html_e('Scan History', 'Site-Add-on-Watchdog-main'); ?>
            </div>
        ...
        <div class="wp-watchdog-surface">
            <div class="wp-watchdog-section-title">
                ...
                <?php esc_html_e('Notification channels', 'Site-Add-on-Watchdog-main'); ?>
            </div>
            <?php
            $watchdogChannels = [
                'email'   => __('Email', 'Site-Add-on-Watchdog-main'),
                'slack'   => __('Slack', 'Site-Add-on-Watchdog-main'),
                'teams'   => __('Microsoft Teams', 'Site-Add-on-Watchdog-main'),
                'discord' => __('Discord', 'Site-Add-on-Watchdog-main'),
                'webhook' => __('Custom Webhook', 'Site-Add-on-Watchdog-main'),
            ];
            ?>
                ...
                <?php foreach ($watchdogChannels as $watchdogChannelKey => $watchdogChannelLabel) : ?>
                    ...
                    <li>
                        <span class="wp-watchdog-badge <?php echo esc_attr($watchdogBadgeClass); ?>">
                            ...
                            <?php echo $watchdogChannelEnabled ? esc_html__('On', 'Site-Add-on-Watchdog-main') : esc_html__('Off', 'Site-Add-on-Watchdog-main'); ?>
                        </span>
                    </li>
                <?php endforeach; ?>
            </ul>
            <p class="wp-watchdog-muted" style="margin-top:12px;">
                <?php esc_html_e('Keep channels enabled to receive instant alerts and download-ready reports.', 'Site-Add-on-Watchdog-main'); ?>
            </p>
        </div>
    </div>
    ...
    <div class="wp-watchdog-surface">
        <div class="wp-watchdog-section-header">
            <div class="wp-watchdog-section-title">
                ...
                <?php esc_html_e('Potential Risks', 'Site-Add-on-Watchdog-main'); ?>
            </div>
            <div class="wp-watchdog-summary">
                ...
                <span class="wp-watchdog-summary__label"><?php esc_html_e('items flagged', 'Site-Add-on-Watchdog-main'); ?></span>
            </div>
        </div>
    <?php if (empty($watchdogRisks)) : ?>
        <p class="wp-watchdog-muted"><?php esc_html_e('No risks detected.', 'Site-Add-on-Watchdog-main'); ?></p>
    <?php else : ?>
        <?php
        $watchdogColumns = [
            'plugin'   => __('Plugin', 'Site-Add-on-Watchdog-main'),
            'local'    => __('Local Version', 'Site-Add-on-Watchdog-main'),
            'remote'   => __('Directory Version', 'Site-Add-on-Watchdog-main'),
            'reasons'  => __('Reasons', 'Site-Add-on-Watchdog-main'),
            'actions'  => __('Actions', 'Site-Add-on-Watchdog-main'),
        ];
        $watchdogPerPage = (int) apply_filters('site_add_on_watchdog_main_admin_risks_per_page', 10);
            ...
            <div class="wp-watchdog-risk-table__controls">
                <form method="get" class="wp-watchdog-risk-table__filter-form">
                    ...
                    <label class="wp-watchdog-risk-table__search">
                        <span class="screen-reader-text"><?php esc_html_e('Search risks', 'Site-Add-on-Watchdog-main'); ?></span>
                        <input
                            type="search"
                            name="risk_search"
                            value="<?php echo esc_attr($watchdogRiskSearch); ?>"
                            placeholder="<?php esc_attr_e('Search plugin or reason', 'Site-Add-on-Watchdog-main'); ?>"
                            data-risk-search
                        />
                    </label>
                    <div class="wp-watchdog-risk-table__sort-select">
                        <label class="screen-reader-text" for="wp-watchdog-risk-sort"><?php esc_html_e('Sort risks by', 'Site-Add-on-Watchdog-main'); ?></label>
                        <select id="wp-watchdog-risk-sort" name="risk_sort" data-risk-sort>
                            ...
                            >
                                <?php esc_html_e('Plugin name', 'Site-Add-on-Watchdog-main'); ?>
                            </option>
                            <option
                                value="risk_count"
                            ...
                            >
                                <?php esc_html_e('Risk count', 'Site-Add-on-Watchdog-main'); ?>
                            </option>
                            <option
                                value="version_gap"
                            ...
                            >
                                <?php esc_html_e('Version gap', 'Site-Add-on-Watchdog-main'); ?>
                            </option>
                            <option
                                value="local"
                            ...
                            >
                                <?php esc_html_e('Local version', 'Site-Add-on-Watchdog-main'); ?>
                            </option>
                            <option
                                value="remote"
                            ...
                            >
                                <?php esc_html_e('Directory version', 'Site-Add-on-Watchdog-main'); ?>
                            </option>
                            <option
                                value="reasons"
                            ...
                            >
                                <?php esc_html_e('Reasons', 'Site-Add-on-Watchdog-main'); ?>
                            </option>
                        </select>
                        <label class="screen-reader-text" for="wp-watchdog-risk-order"><?php esc_html_e('Sort order', 'Site-Add-on-Watchdog-main'); ?></label>
                        <select id="wp-watchdog-risk-order" name="risk_order" data-risk-order>
                            <option value="asc" <?php selected($watchdogRiskOrderSelection, 'asc'); ?>>
                                <?php esc_html_e('Ascending', 'Site-Add-on-Watchdog-main'); ?>
                            </option>
                            <option value="desc" <?php selected($watchdogRiskOrderSelection, 'desc'); ?>>
                                <?php esc_html_e('Descending', 'Site-Add-on-Watchdog-main'); ?>
                            </option>
                        </select>
                        <button class="button" type="submit"><?php esc_html_e('Apply', 'Site-Add-on-Watchdog-main'); ?></button>
                    </div>
                </form>
                <div class="wp-watchdog-risk-table__pagination" data-pagination>
                    <button type="button" class="button" data-action="prev" aria-label="<?php esc_attr_e('Previous page', 'Site-Add-on-Watchdog-main'); ?>" disabled>&lsaquo;</button>
                    <span class="wp-watchdog-risk-table__page-status" data-page-status></span>
                    <button type="button" class="button" data-action="next" aria-label="<?php esc_attr_e('Next page', 'Site-Add-on-Watchdog-main'); ?>" disabled>&rsaquo;</button>
                </div>
            </div>
            <div class="wp-watchdog-risk-table__scroll">
                <table class="widefat fixed striped wp-list-table">
                    ...
                    <?php foreach ($watchdogRisks as $watchdogRisk) : ?>
                        <?php
                        $watchdogRemoteVersion = $watchdogRisk->remoteVersion ?? __('N/A', 'Site-Add-on-Watchdog-main');
                        ...
                        >
                            ...
                            <td data-column="reasons" data-column-label="<?php echo esc_attr($watchdogColumns['reasons']); ?>">
                                <ul>
                                    ...
                                    <?php if (! empty($watchdogRisk->details['vulnerabilities'])) : ?>
                                        <li>
                                            <?php esc_html_e('WPScan vulnerabilities:', 'Site-Add-on-Watchdog-main'); ?>
                                            <ul>
                                                    ...
                                                    <li>
                                                        ...
                                                        <?php if (! empty($watchdogVuln['fixed_in'])) : ?>
                                                            ...
                                                            printf(
                                                                ...
                                                                esc_html__('Fixed in %s', 'Site-Add-on-Watchdog-main'),
                            ...
                            <td data-column="actions" data-column-label="<?php echo esc_attr($watchdogColumns['actions']); ?>">
                                <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                                    ...
                                    <button class="button" type="submit"><?php esc_html_e('Ignore', 'Site-Add-on-Watchdog-main'); ?></button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
    ...
    <div class="wp-watchdog-grid">
        <div class="wp-watchdog-surface">
            <div class="wp-watchdog-section-title">
                ...
                <?php esc_html_e('Ignored Plugins', 'Site-Add-on-Watchdog-main'); ?>
            </div>
            <?php if (empty($watchdogIgnored)) : ?>
                <p class="wp-watchdog-muted"><?php esc_html_e('No plugins are being ignored.', 'Site-Add-on-Watchdog-main'); ?></p>
            <?php else : ?>
                <ul class="wp-watchdog-inline-list">
        ...
        <div class="wp-watchdog-surface">
            <div class="wp-watchdog-section-title">
                ...
                <?php esc_html_e('Notifications', 'Site-Add-on-Watchdog-main'); ?>
            </div>
    <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
        ...
        <table class="form-table" role="presentation">
            <tr>
                <th scope="row"><?php esc_html_e('History retention', 'Site-Add-on-Watchdog-main'); ?></th>
                <td>
                    <label for="wp-watchdog-history-retention" class="screen-reader-text"><?php esc_html_e('History retention', 'Site-Add-on-Watchdog-main'); ?></label>
                    <input
                        type="number"
                    ...
                    />
                    <p class="description">
                        <?php esc_html_e('Number of recent scans to keep available for review and download (maximum 15).', 'Site-Add-on-Watchdog-main'); ?>
                    </p>
                </td>
            </tr>
            <tr>
                <th scope="row"><?php esc_html_e('Scan frequency', 'Site-Add-on-Watchdog-main'); ?></th>
                <td>
                    <?php
                    $watchdogDefaultFrequencyMessage = __('Choose how often the automatic scan should run.', 'Site-Add-on-Watchdog-main');
                    $watchdogTestingFrequencyMessage = sprintf(
                        /* translators: 1: interval minutes, 2: duration hours */
                        __('Testing mode sends notifications every %1$d minutes to all configured channels and automatically switches back to daily scans after %2$d hours.', 'Site-Add-on-Watchdog-main'),
                    ...
                    $watchdogWeeklyTime = isset($watchdogSettings['notifications']['weekly_time'])
                        ...
                        : '08:00';
                    ?>
                    <label for="wp-watchdog-notification-frequency" class="screen-reader-text"><?php esc_html_e('Scan frequency', 'Site-Add-on-Watchdog-main'); ?></label>
                    <select id="wp-watchdog-notification-frequency" name="settings[notifications][frequency]">
                        <option value="daily" <?php selected($watchdogSettings['notifications']['frequency'], 'daily'); ?>><?php esc_html_e('Daily', 'Site-Add-on-Watchdog-main'); ?></option>
                        <option value="weekly" <?php selected($watchdogSettings['notifications']['frequency'], 'weekly'); ?>><?php esc_html_e('Weekly', 'Site-Add-on-Watchdog-main'); ?></option>
                        <option value="testing" <?php selected($watchdogSettings['notifications']['frequency'], 'testing'); ?>><?php echo esc_html(
                            sprintf(
                                ...
                                __('Testing (every %d minutes)', 'Site-Add-on-Watchdog-main'),
                                TestingMode::INTERVAL_MINUTES
                            )
                        ); ?></option>
                        <option value="manual" <?php selected($watchdogSettings['notifications']['frequency'], 'manual'); ?>><?php esc_html_e('Manual (no automatic scans)', 'Site-Add-on-Watchdog-main'); ?></option>
                    </select>
                    <?php
                    ...
                    <div class="wp-watchdog-frequency-options" data-watchdog-frequency-options>
                        <div class="wp-watchdog-frequency-options__row" data-watchdog-frequency-target="daily">
                            <label for="wp-watchdog-daily-time" class="wp-watchdog-frequency-label">
                                <?php esc_html_e('Daily send time', 'Site-Add-on-Watchdog-main'); ?>
                            </label>
                            <input
                            ...
                            <p class="description" id="wp-watchdog-daily-time-help">
                                <?php esc_html_e('Time of day to start the daily scan (server timezone).', 'Site-Add-on-Watchdog-main'); ?>
                            </p>
                        </div>
                        <div class="wp-watchdog-frequency-options__row" data-watchdog-frequency-target="weekly">
                            <div class="wp-watchdog-frequency-weekly">
                                <label for="wp-watchdog-weekly-day" class="wp-watchdog-frequency-label">
                                    <?php esc_html_e('Weekly send day', 'Site-Add-on-Watchdog-main'); ?>
                                </label>
                                <select id="wp-watchdog-weekly-day" name="settings[notifications][weekly_day]">
                                    <option value="1" <?php selected($watchdogWeeklyDay, 1); ?>><?php esc_html_e('Monday', 'Site-Add-on-Watchdog-main'); ?></option>
                                    <option value="2" <?php selected($watchdogWeeklyDay, 2); ?>><?php esc_html_e('Tuesday', 'Site-Add-on-Watchdog-main'); ?></option>
                                    <option value="3" <?php selected($watchdogWeeklyDay, 3); ?>><?php esc_html_e('Wednesday', 'Site-Add-on-Watchdog-main'); ?></option>
                                    <option value="4" <?php selected($watchdogWeeklyDay, 4); ?>><?php esc_html_e('Thursday', 'Site-Add-on-Watchdog-main'); ?></option>
                                    <option value="5" <?php selected($watchdogWeeklyDay, 5); ?>><?php esc_html_e('Friday', 'Site-Add-on-Watchdog-main'); ?></option>
                                    <option value="6" <?php selected($watchdogWeeklyDay, 6); ?>><?php esc_html_e('Saturday', 'Site-Add-on-Watchdog-main'); ?></option>
                                    <option value="7" <?php selected($watchdogWeeklyDay, 7); ?>><?php esc_html_e('Sunday', 'Site-Add-on-Watchdog-main'); ?></option>
                                </select>
                            </div>
                            <div class="wp-watchdog-frequency-weekly">
                                <label for="wp-watchdog-weekly-time" class="wp-watchdog-frequency-label">
                                    <?php esc_html_e('Weekly send time', 'Site-Add-on-Watchdog-main'); ?>
                                </label>
                                <input
                            ...
                            <p class="description" id="wp-watchdog-weekly-time-help">
                                <?php esc_html_e('Day and time to start the weekly scan (server timezone).', 'Site-Add-on-Watchdog-main'); ?>
                            </p>
                        </div>
                    </div>
                    <?php if ($watchdogShowTestingExpiry) : ?>
                        ...
                        $watchdogTestingExpiryMessage = sprintf(
                            ...
                            __('Testing mode will automatically switch back to daily scans on %1$s (%2$s remaining).', 'Site-Add-on-Watchdog-main'),
                            wp_date(
            ...
            <tr>
                <th scope="row"><?php esc_html_e('Email notifications', 'Site-Add-on-Watchdog-main'); ?></th>
                <td data-watchdog-notification>
                    <label class="wp-watchdog-notification-toggle">
                        ...
                        <?php esc_html_e('Enabled', 'Site-Add-on-Watchdog-main'); ?>
                    </label>
                    <div class="wp-watchdog-notification-fields" data-watchdog-fields>
                        <label>
                            <?php esc_html_e('Recipients (comma separated)', 'Site-Add-on-Watchdog-main'); ?><br />
            ...
            <tr>
                <th scope="row"><?php esc_html_e('Discord notifications', 'Site-Add-on-Watchdog-main'); ?></th>
                <td data-watchdog-notification>
                    <label class="wp-watchdog-notification-toggle">
                    ...
                    <div class="wp-watchdog-notification-fields" data-watchdog-fields>
                        <label>
                            <?php esc_html_e('Discord webhook URL', 'Site-Add-on-Watchdog-main'); ?><br />
            ...
            <tr>
                <th scope="row"><?php esc_html_e('Slack notifications', 'Site-Add-on-Watchdog-main'); ?></th>
                <td data-watchdog-notification>
                    <label class="wp-watchdog-notification-toggle">
                    ...
                    <div class="wp-watchdog-notification-fields" data-watchdog-fields>
                        <label>
                            <?php esc_html_e('Slack webhook URL', 'Site-Add-on-Watchdog-main'); ?><br />
            ...
            <tr>
                <th scope="row"><?php esc_html_e('Microsoft Teams notifications', 'Site-Add-on-Watchdog-main'); ?></th>
                <td data-watchdog-notification>
                    <label class="wp-watchdog-notification-toggle">
                    ...
                    <div class="wp-watchdog-notification-fields" data-watchdog-fields>
                        <label>
                            <?php esc_html_e('Teams webhook URL', 'Site-Add-on-Watchdog-main'); ?><br />
            ...
            <tr>
                <th scope="row"><?php esc_html_e('Generic webhook', 'Site-Add-on-Watchdog-main'); ?></th>
                <td data-watchdog-notification>
                    <label class="wp-watchdog-notification-toggle">
                    ...
                    <div class="wp-watchdog-notification-fields" data-watchdog-fields>
                        <label>
                            <?php esc_html_e('Webhook URL', 'Site-Add-on-Watchdog-main'); ?><br />
                        ...
                        <p>
                            <label>
                                <?php esc_html_e('Webhook secret (optional)', 'Site-Add-on-Watchdog-main'); ?><br />
                            ...
                            <span class="description"><?php esc_html_e('Used to sign webhook payloads with an HMAC signature.', 'Site-Add-on-Watchdog-main'); ?></span>
                        </p>
                    </div>
                </td>
            </tr>
            <tr>
                <th scope="row"><?php esc_html_e('WPScan API key', 'Site-Add-on-Watchdog-main'); ?></th>
                <td>
                    ...
                    <p class="description"><?php esc_html_e('Optional. Provide your own WPScan API key to enrich vulnerability reports.', 'Site-Add-on-Watchdog-main'); ?></p>
                </td>
            </tr>
        </table>
        <p><button class="button button-primary" type="submit"><?php esc_html_e('Save settings', 'Site-Add-on-Watchdog-main'); ?></button></p>
    </form>
        </div>
    </div>
</div>
